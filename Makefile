...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=duke \
	-X github.com/cosmos/cosmos-sdk/version.AppName=duked \
...
install: validate-go-version go.sum
	@echo "--> Installing duked"
	@echo "go install -mod=readonly $(BUILD_FLAGS) ./cmd/duked"
	@go install -mod=readonly $(BUILD_FLAGS) ./cmd/duked

build: validate-go-version go.sum
	@echo "--> Build duked"
	@go build -mod=readonly $(BUILD_FLAGS) -o ./build/duked ./cmd/duked
...
release:
	...
	$(DOCKER) run \
		...
		-v `pwd`:/go/src/github.com/conan321/duke \
		-w /go/src/github.com/conan321/duke \
